...
# -*- coding: utf-8 -*-
"""
深度分析 2026-09-19 (周六非交易日, 复用 2026-09-18 选股报告)
按 9/12 + 9/13 + 9/14 + 9/15 + 9/16 + 9/17 + 9/18 深度分析 SOP + 6 维评分

⚠️ 重要 — 9/19 周六非交易日 (无 9/19 选股报告), 复用 9/18 (周五) 选股报告:
- 9/19 周六 cron 触发 — 但 A 股周末休市, 无新选股报告
- 沿用 9/13 (周日复用 9/11) 模式 — 复用最近一个交易日 (9/18 周五) 报告
...
TODAY = "2026-09-19"          # 今天周六非交易日
RUN_ID = "01a0b712-3f77-77e8-9f49-a4074116e080"

# 9/19 复用 9/18 候选池 (Top 10, 从 2026-09-18 选股报告 Top 10 提取)
...
def map_to_phase10_sector(industry):
    """9/19 板块映射: 完全沿用 9/18 (复用 9/13-9/18 全部映射)
...
def main():
    print(f"\n{'='*80}")
    print(f"AAna 深度分析 — {REPORT_DATE} 选股报告 (复用, 9/19 周六非交易日)")
    print(f"今日 {TODAY} 周六非交易日 — 沿用 9/13 (周日复用 9/11) 模式")
    ...
    print(f"   ⚠️ 今日 9/19 周六非交易日, 无新选股报告, 复用 9/18 (周五) 报告")
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump({
            ...
            "today": TODAY,
            "today_weekday": "周六 (非交易日)",
            "reuse_note": "周六非交易日无新选股报告, 沿用 9/13 模式复用最近交易日 (9/18 周五) 报告",
            "run_id": RUN_ID,
