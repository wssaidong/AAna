...
def main():
    ...
    #   v2: 传 all_feedback (CSV 全表),由 _duckdb_crosscheck 自行按 30 日口径过滤对比
    _duckdb_crosscheck(all_feedback)
...
def _duckdb_crosscheck(all_feedback_rows):
    """Phase 8C: DuckDB 同口径交叉验证 — 不阻断,只是 stderr warn。

    v2 (2026-08-23): 修正口径 bug — v1 拿 compute_stats(top_n=20 的"末尾20条")
    与 DuckDB 30 日窗口比,样本天然不同(diff 1.1pp 假警报)。
    现在传 all_feedback_rows (CSV 全表),自行用 30 日窗口过滤后与 DuckDB 对比,
    两侧口径严格一致: ret_1d 非空 + rec_date >= today-30。

    容忍: |pandas_wr - duckdb_wr| < 0.5pp 且 n 相同。
    """
    try:
        ...
        for r in all_feedback_rows or []:
            ...
            if rd >= cutoff and v is not None:
                sample.append(v)
        pandas_n = len(sample)
        pandas_wr = round(100.0 * sum(1 for v in sample if v > 0) / pandas_n, 1) if pandas_n else 0.0

        db_wr = db.get("win_rate", 0.0)
        db_n = db.get("n", 0)
        diff = abs(pandas_wr - db_wr)
        if diff > 0.5 or pandas_n != db_n:
            print(
                f"   [Phase 8C] ⚠️  不一致: pandas(30d, n={pandas_n}, wr={pandas_wr}%) "
                f"vs DuckDB(n={db_n}, wr={db_wr}%), diff={diff:.1f}pp",
                file=sys.stderr,
            )
        else:
            print(
