...
# -*- coding: utf-8 -*-
"""
深度分析 2026-09-18 选股报告 (周五交易日, 10 只候选 — 分歧行情)
按照 9/12 + 9/13 + 9/14 + 9/15 + 9/16 + 9/17 深度分析 SOP + 6 维评分
沿用 9/13 (水电→energy) + 9/15 (锂→energy) + 9/16 (海运→transport / 稀土+海缆+氟化工→energy)
     + 9/17 (锑-光伏→energy) 映射

⚠️ 重要 — 9/18 报告注意事项:
- 情绪 50 (分歧) — vs 9/17 80 (亢奋) / 9/16 40 (分歧) — 行情强度回落
- 上证 -0.41% 下跌 — 建议仓位 50% (报告默认)
- 10 只 Top 候选 (重点关注 Top 10, 充裕)
- 候选大多落在电子/小金属 — 警惕黑名单
- 9/18 候选池整体偏电子, 需重点识别是否有 energy 板块候选
- 9/17 复盘: 3/10=30.0% 命中 V2.5 终结连续 2 个温和正 Alpha 日 (净累计 +0.83pp)
- 整体均值 -1.28% vs 上证 -0.41% 超额 -0.87pp
- 7/10 负延续 Δ均值 -5.38pp + 0/10 主线重合度 🆕史上最差
- 上证 3875.60 -0.41% 4大核心平均 -0.325%
- 9/18 行情特征: 假亢奋真回吐·早盘 +0.71% → 收盘 -0.41% 反转 -1.12pp
- 9/18 情绪回到分歧, 候选质量需重点识别
...
- 机械/军工 → mach (0%)
- 海运/航运 → transport (无样本)
- 9/18 新增考虑 — 春光科技 (吸尘器/家电) / 雷柏科技 (无线键鼠) / 融捷股份 (锂) / 八方股份 (电踏车) / 东睦股份 (粉末冶金) / 百合花 (颜料) / 中京电子 (PCB) / 科博达 (汽车电子)
"""
import sys
import json
import os
...
TODAY = "2026-09-18"
RUN_ID = "01a0b1eb-c083-7e19-b739-0b1f28b00f0b"

# 9/18 候选池 (Top 10, 从 2026-09-18 选股报告 Top 10 提取)
...
def map_to_phase10_sector(industry):
    """9/18 板块映射: 沿用 9/13-9/17 全部映射 + 9/18 候选新增映射
    energy (绿电/储能/锂电/稀土/海缆/锑) — 100% 胜率
...
def main():
    ...
    print(f"今日 {TODAY} 周五交易日 (autopilot 08:30 触发)")
