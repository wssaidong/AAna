...
# -*- coding: utf-8 -*-
"""
深度分析 2026-09-21 (周一, 复用 2026-09-18 选股报告 — 9/19/20/21 周末无新报告)
按 9/12 + 9/13 + 9/14 + 9/15 + 9/16 + 9/17 + 9/18 + 9/19 + 9/20 深度分析 SOP + 6 维评分

⚠️ 重要 — 9/21 周一 (复用 9/18 选股报告):
- 9/19 周六/9/20 周日 均沿用 9/18 (周五) 选股报告, AAna 主流程未生成新报告 (周末休市)
- 9/21 周一 08:30 autopilot 触发时, AAna 主流程 9/21 选股报告尚未生成 (17:01 才生成)
- 沿用 9/13 (周日复用 9/11) + 9/19/9/20 模式 — 复用最近交易日 (9/18 周五) 报告
...
- 9/19 + 9/20 深度分析结果: 八方股份 Top 1 (62.50) + 融捷股份 Top 2 (44.56) — 组合 20260919ZZ + 20260920ZZ
- 9/21 继续验证 energy 板块 100% 胜率 + 9/18 选股报告 vs 9/21 周一开盘价的价格印证
...
- 9/21 复用 9/18 全部映射 (与 9/19/9/20 一致)
"""
import sys
import json
import os
...
TODAY = "2026-09-21"          # 今天周一
RUN_ID = "01a0bf9a-cd73-780e-b7c2-6a1cd3cd2e62"

# 9/21 复用 9/18 候选池 (Top 10, 从 2026-09-18 选股报告 Top 10 提取 — 与 9/19/9/20 一致)
...
def map_to_phase10_sector(industry):
    """9/21 板块映射: 完全沿用 9/18 (复用 9/13-9/20 全部映射)
...
def main():
    print(f"\n{'='*80}")
    print(f"AAna 深度分析 — {REPORT_DATE} 选股报告 (复用, 9/21 周一交易日前)")
    print(f"今日 {TODAY} 周一交易日 — 沿用 9/13 (周日复用 9/11) + 9/19 (周六复用 9/18) + 9/20 (周日复用 9/18) 模式")
    ...
    print(f"   ⚠️ 今日 9/21 周一交易日, 9/21 选股报告尚未生成 (17:01 才生成), 复用 9/18 (周五) 报告")
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump({
            ...
            "today": TODAY,
            "today_weekday": "周一 (交易日, 9/21 选股报告尚未生成)",
            "reuse_note": "周一交易日, 9/21 选股报告 17:01 收盘后生成, 9/21 早 08:30 autopilot 触发时复用 9/18 周五报告 (沿用 9/13/9/19/9/20 复用模式)",
            "run_id": RUN_ID,
